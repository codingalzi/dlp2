...
# **업샘플링** 모듈(upsampling module)으로 이루어진다.
# 
# - 다운샘플링 모듈: `Conv2D` 층 활용
#     - 이미지 분류 모델과 동일한 기능.
...
# 여기서 모듈은 여러 개의 층<font size='2'>layer</font>으로 구성되며, 하나의 모듈이 여러 번 
# **재활용**<font size='2'>reuse</font>되기도 한다. 
# 예를 들어, {numref}`%s장 <ch:computer-vision-intro>`에서 다룬 VGG16 모델은 "Conv2D, Conv2D, MaxPooling2D" 로 구성된 모듈을 
# 계층으로 구성하였다. 
# 
...
# 모듈의 입력값을 모델을 통과하여 생성된 출력값과 합쳐서 다음 모델으로 전달하는 아키텍처이다(아래 그림 참고).
