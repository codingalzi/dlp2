...
# ```
# 
# `train`의 `pos`와 `neg` 서브디렉토리에 각각 12,500 개의 긍정과 부정 리뷰가
...
# 먼저 훈련셋의 모든 리뷰 문장을 정수들의 벡터로 변환한다.
# 단, 리뷰 문장이 최대 600 개의 단어만 포함하도록 한다. 
# 또한 사용되는 어휘는 빈도 기준 최대 2만 개로 제한한다. 
# 후기의 길이를 600 개의 단어로 제한한 이유는 리뷰가 평균적으로 233 개의 단어를 사용하고,
