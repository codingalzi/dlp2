...
# 
# 문장에 포함된 단어들의 순서를 있는 그대로 전달받아 문장에 포함된 특성을 스스로 
# 찾아내는 다양한 딥러닝 순차 모델의 활용법을 소개한다.

# **순차 모델 예제 1: 순환 신경망 활용**

# 양방향 LSTM 층으로 구성된 순차 모델을 IMDB 영화 후기 데이터셋으로 훈련시켜 보자.
# 그러려면 먼저 영화 후기 문장을 적절한 시퀀스로 변환해야 한다.
...
# `[2, 1, 4, 0, 0]`의 각 항목을 원-핫 벡터로 변환한 값들로 이뤄진 시퀀스는 다음과 같다.
# 
# ```python
# [[0, 0, 1, 0, 0],      # 2
#  [0, 1, 0, 0, 0],      # 1
...
# ```
# 
# `tf.one_hot()` 함수를 다음과 같이 호출하면 위 결과를 생성한다.
# 
# ```python
...
# 이렇게 하면 정수 벡터를 바로 모델의 입력값으로 사용해서 훈련을 시킬 수 있다.
...
# 테스테셋에 대한 정확도가 87% 정도에 불과해서 바이그램 모델보다 성능이 낮다.

# **순차 모델 예제 2: 단어 임베딩 활용**
