...
# 케라스의 `Embedding` 층은 하나의 문장이 입력되면 
...
# 압축파일의 크기는 822 MB이다.
# 다운로드한 파일의 압축을 풀면 다음 세 개의 파일이 생성된다.
# 
...
# 화면상 10여 개의 값만 보여지고 나머지는 화면에서 잘렸다.
...
# - `embedding_matrix`: `glove.6B.100d.txt` 파일에 포함된 어휘 토큰 중에서 
#     훈련셋에 포함된 2만개의 단어에 해당하는 어휘만 추출해서 생성한 
#     `(20000, 100)` 모양의 임베딩 행렬이다.
#     이 행렬을 어휘 색인으로 참고하여 후기에 사용된 각 단어에 대해 하나의 단어 벡터를 연결한다.
# - `Constant`: 단어 임베딩의 기준으로 사용될 행렬을 상수 텐서로 지정한다.
# - `trainable=False`: 단어 임베딩의 기준이 되는 임베딩 행렬이 훈련되지 않도록 한다.
...
#     - 1단계: 문장에 사용된 각 토큰들 사이의 연관성을 계산하여 어텐션 점수<font size='2'>attention scores</font> 생성.
...
# 아래 `self_attention()` 함수가 셀프 어텐션이 작동하는 과정을 잘 보여준다.
# 함수의 입력값으로 단어 임베딩된 시퀀스를 사용한다.
...
# 이후 트랜스포머 인코더로 셀프 어텐션을 적용하여 단어가 사용되는 문장에서의 문맥상의 특성을 추가한다.
# 
# 사용되는 변수들은 다음과 같다.
# 
...
# 
# - ("훈련셋의 크기" $/$ "문장의 단어 수 평균") $>$ 1500 인 경우: 트랜스포머 등 순차 모델
# - ("훈련셋의 크기" $/$ "문장의 단어 수 평균") $<$ 1500 인 경우: 바이그램 단어주머니 모델
