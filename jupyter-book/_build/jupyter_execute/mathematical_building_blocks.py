...
# **텐서의 차원**
# 
# 텐서의 **차원**은 어레이 등 텐서의 표현에 사용된 **축**<font size='2'>axis</font>의 수로 결정된다.
# **랭크**<font size='2'>rank</font>라 불리기도 한다.

# - 0차원(0D) 텐서: 정수 한 개, 부동소수점 한 개 등 하나의 수를 표현하는 텐서. 
#     **스칼라**<font size='2'>scalar</font>라고 불림.
#     ```
#     np.array(12)
#     np.array(1.34)
#     ```
...
#     **벡터**<font size='2'>vector</font>라고도 불림.
#     한 개의 축을 가짐    
#     ```
#     np.array([12, 3, 6, 14, 7])
#     ```

# - 2차원(2D) 텐서: 행<font size='2'>row</font>과 열<font size='2'>column</font> 두 개의 축을 가짐. 
#     **행렬**<font size='2'>matrix</font>이라고도 불림.
#     ```
...
# **텐서 속성**
# 
# 텐서의 주요 속성 세 가지는 다음과 같으며, 넘파이 어레이와 동일한 기능을 갖는다.
# 
# - `ndim` 속성: 차원 수(랭크) 저장. 
...
# - `shape` 속성: 축 별 크기.
#     예를 들어 MNIST는 3개의 축으로 구성되며
#     0번 축은 샘플 수, 1번 축은 이미지의 행의 수, 2번 축은 이미지의 열의 수를 가리킴.
#     ```python
#     >>> train_images.shape
#     (60000, 28, 28)
#     ```
...
#     예를 들어, MNIST 훈련셋에 포함된 이미지의 픽셀은 0과 255 사이의 정수를
#     가리키는 `unit8` 자료형 사용.
#     ```python
#     >>> train_images.dtype
#     uint8
...
#         `(3,)` 모양의 벡터로 표현.
#     - 10만 명의 통계 데이터를 포함한 데이터셋은 `(100000, 3)` 모양의 2D 텐서로 표현.
# - 예제 2
#     - 샘플: 특정 문서에서 2만 개의 단어 각각이 사용된 빈도수로 구성된 데이터.
#         `(20000,)` 모양의 벡터로 표현.
#     - 500개의 문서를 대상으로 할 경우 `(500, 20000)` 모양의 2D 텐서로 표현.

# **3D 텐서 예제: 시계열 또는 순차 데이터 활용**
# 
...
# - 예제 1
#     - 샘플: 하루 390번 매 분마다 측정한 (현재 증시가, 지난 1분 동안 최고가, 지난 1분 동안 최저가) 데이터.
#         `(390, 3)` 모양의 2D 텐서로 표현.        
...
#         각 항목은 0 또는 1.
#     - 백만 개의 트윗은 `(1000000, 280, 128)` 모양의 3D 텐서로 표현.

# **4D 텐서 예제: 이미지 데이터 활용**
# 

# - 이미지 데이터셋: `(샘플 수, 가로, 세로, 채널 수)` 또는 `(샘플 수, 채널 수, 가로, 세로)` 모양의 4D 텐서
...
# - 동영상 데이터셋: `(샘플 수, 프레임 수, 가로, 세로, 채널 수)` 모양의 5D 텐서
