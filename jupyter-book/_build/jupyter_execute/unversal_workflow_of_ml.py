...
#     이경우엔 메모리, 전력, 공간 관련 제약이 없어서 훈련된 모델을 그대로 활용 가능.

# 보통 아래 상황에서 REST API 대신 직접 장치에서 활용한다.
# 
# - 모델이 빠르게 반응해야 하는 경우
# - 인터넷이 충분히 빠르지 않은 경우: 몰입형 증강 현실 앱이 외부 서버를 이용하려면 부하가 너무 커진다.
# - 메모리와 전력 제한이 있는 장치에서도 충분히 잘 돌아갈 수 있으 정도로 모델을 충분히 작게 만들수 있는 경우.
...
#     메모리와 전력이 약하다면 좋은 GPU를 이용하여 훈련된 모델을 그대로 사용할 수 없다.
...
# 앞서 언급한 대로 실전 환경에 따라 장치의 메모리와 전력, 인터넷 속도,
# 매우 빠른 반응 속도 등의 제한 요인이 발생한다.
# 따라서 사용 환경에 따른 학습된 모델의 최적화를 먼저 진행한 후에 실전에 배치해야 한다.
# 모델 최적화는 TensorFlow.js, TensorFlow Lite 등을 이용하기 이전에 진행한다.

# 일반적으로 아래 두 가지 방식을 이용하여 모델 최적화를 진행한다.
# 
# - 가중치 가지치기<font size ='2'>weight pruning</font>: 모델 예측에 가중치의 기여도가 각각 다르다.
#     따라서 적절한 크기 이상의 가중치만 고려해도 성능이 심하게 약해지지 않을 수 있다. 
#     기여도가 낮은 가중치는 0으로 만드는 과정을 가중치 가지치기라 한다.
# 
# - 가중치 양자화<font size='2'>weight quantization</font>: 
#     딥러닝 모델 훈련은 기본적으로 `float32` 자료형을 이용하는 
#     단 정밀도<font size='2'>single precision</font> 계산을 수행한다.
#     하지만 훈련된 모델을 단순히 예측을 위해, 즉 순전파 용도로만
#     사용하면 가중치를 8-비트 정수(`int8`)로 변환해서 사용해도 
#     모델 예측의 정확도는 별로 변하지 않는다.

# [케라스가 인퍼런스 모델 최적화](https://www.tensorflow.org/model_optimization)를 지원한다.

# ### 실전 배치 모델 모니터링

# 모델의 실전 배치를 완료한 이후에도 아래 사항들을 확인하기 위해 지속적으로 모델의 성능을 지켜봐야 한다.
# 
# - 옳바른 작동 여부
# - 새로운 데이터에 대한 성능
# - 기타 앱들과의 상호 작용
# - (의뢰인) 사업 지표에의 영향 정도
# 
# 실전 배치된 모델을 모니터링 하기 위해 모델의 작동상태를 기록하는 코드도 함께
# 완성된 제품에 미리 심어 놓아야 한다.
# 또한 경우에 따라 다음과 같은 추가 작업도 실행한다.
# 
# - 새로 도입한 음악 추천 시스템, 광고 클릭율 예측 시스템의 성능이 이전 모델보다 향상되었는가?
#     이를 위해 A/B 테스팅 기법을 적용할 수 있다.
# 
# - 가능하면 모델 예측값의 정확도를 직접 확인한다. 새 데이터에 대한 예측값을 직접 확인하여 모델의 성능을 확인한다. 이미지 탐색 엔진, 비정상 과자 탐지기 등에 대해 반드시 이 작업을 수행해야 한다.
# 
# - 직접 확인이 불가능하다면 시스템 활용자들의 활용 데이터를 수집할 수 있다.
#     예를 들어, 스팸 필터, 공격적인 내용을 담은 메일 확인 시스템 등에 대해 이 방식을 사용한다.

# ### 모델 유지 보수

# 어떤 모델도 영원히 사용될 수 없다. 
# 이유는 시간 흐르고 환경이 변하면 모델의 입력 데이터에 포함된 정보도 함께 변하기 때문이다.
# 이런 현상을 콘셉 드리프트<font size='2'>concept drift</font>라 한다.
# 
# 예를 들어, 음악 추천 시스템은 몇 주 정도, 신용카드 부정 사용 탐지 시스템은 며칠 정도, 
# 이미지 검색 엔진은 최대 몇 년 정도 사용하면 시스템을 업데이트/업그레이드 해야 한다.

# 모델 출신 이후부터 지속적으로 아래 작업과 같은 일을 수행해야 한다.
# 
# - 제품에 사용되는 데이터 상의 변화를 감시한다.
#     새로운 특성이 추가되었는가? 레이블셋을 변경 또는 확장해야 하는가?
# 
# - 데이터 수집과 레이블링을 지속해야 한다. 또한 레이블링 파이프라인을 계속해서 개선해야 한다.
#     특히, 현재 모델이 어려워하는 샘플을 수집해서 모델의 성능을 개선하는 데에 사용해야 한다.
