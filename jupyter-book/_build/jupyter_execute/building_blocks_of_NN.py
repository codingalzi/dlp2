...
# **그레이디언트 테이프**
# 
# 텐서플로우의 `GradientTape` 클래스의 객체를 이용하여 손실함수의 그레이디언트를 자동으로
# 미리 계산해두고 역전파 등 필요할 때 활용한다.
...
# `tf.Variable` 클래스는 항목이 변할 수 있는 텐서 객체를 생성할 때 사용하며
# 가중치와 편향 등 신경망 모델의 파라미터와 손실함수에 사용되는 변수 등은
# 모두 `tf.Variable` 클래스의 객체로 선언돼야 한다.
